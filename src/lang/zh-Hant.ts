export const languageChineseTraditional = {
    ...
    "help": {
        ...
        "customPromptTemplateToggle": "可在此處設定自定義提示詞切換功能。使用 `<toggle variable>=<toggle name>` 格式，每行一個，例如：`cot=Toggle COT`。您可以在提示詞中透過 `{{getglobalvar::toggle_<toggle variable>}}` 語法來使用這些切換功能，如：`{{getglobalvar::toggle_cot}}`。",
        ...
        "v2Calculate": "計算數學表達式，支援基本運算（+, -, *, /, %, ^）、比較運算符（<, >, <=, >=, =, !=）、邏輯運算符（&&, ||, !）、優先順序括號以及使用 $variableName 格式的變數替換。變數將自動轉換為數字（無效則默認為 0）。",
        ...
        "experimentalChatCompressionDesc": "壓縮未使用的聊天資料並儲存在單獨的檔案中。這將大幅減少聊天資料的大小，並顯著提升效能，但這是實驗性功能，可能會不穩定，並導致備份功能等出現問題。",
    ...
    "playground": {
        ...
        "inlayImageGallery": "內聯圖片畫廊",
        "inlayImageGalleryEmptyDesc": "在聊天中附加或生成的圖片將顯示在這裡",
        "inlaySelectAll": "全選",
    ...
    "enableBlockPartialEdit": "啟用區塊部分編輯（懸停以編輯個別區塊）",
    "enableDragPartialEdit": "啟用拖曳部分編輯（選取文字以編輯）",
    ...
    "goToChat": "前往聊天",
    "duplicatePluginFoundUpdateIt": "偵測到重複的外掛程式。您是否要更新現有的外掛程式？",
    "loadMore": "載入更多",
    "pluginUpdateFoundInstallIt": "發現插件更新。是否要安裝此更新？",
    "noRequestLogs": "無請求紀錄。",
    "translateCode": "轉換程式碼位址",
    "branchedText": "此對話已從 {} 分支。",
    "disableMessage": "停用訊息",
    "bookmark": "書籤",
    "branch": "分支",
    "partialBackupFirstConfirm": "⚠️ 警告：部分本地備份 ⚠️\n\n此備份僅快速儲存必要資訊。\n\n包含：\n- 資料庫（聊天記錄、角色、模組、外掛、提示詞、設定等）\n- 角色頭像（主要圖片）\n- 使用者頭像及自訂背景\n- 使用者肖像圖示\n- 資料夾圖片\n- 機器人預設圖片\n\n⚠️ 不包含：\n- 表情圖片\n- 額外角色資源\n- VITS 語音檔案\n- 所有其他額外媒體檔案\n\n此備份不包含所有角色資源！\n如需完整備份，請使用一般本地備份。\n\n是否繼續？",
    "partialBackupSecondConfirm": "⚠️⚠️ 最終警告 ⚠️⚠️\n\n此備份僅儲存角色的頭像圖片，不儲存大多數其他資源！\n\n將儲存：\n- 資料庫（聊天記錄、角色、模組、外掛、提示詞、設定等）\n- 角色頭像（主要圖片）\n- 使用者頭像及自訂背景\n- 使用者肖像圖示\n- 資料夾圖片\n- 機器人預設圖片\n\n不會儲存：\n- 表情圖片\n- 額外角色資源\n- VITS 語音檔案\n- 所有其他媒體檔案\n\n⚠️ 僅在備份檔案不需要角色資源時才繼續！⚠️\n\n您真的確定要進行部分備份嗎？",
    ...
    "partialEdit": {
        ...
        "deleteNo": "取消",
        "editModalTitle": "部分修改",
        "matchFound": (method: string) => `符合：${method}`,
        "matchFailedTitle": "無法進行部分修改",
